...
@app.route('/wqi-r', methods=['GET'])
def wqir():
    rivers = DF['RIVER'].unique()
    return render_template('wqir.html', rivers=rivers)
...
@app.route('/stations-r/<river>', methods=['GET'])
def get_stationsr(river):
    stations = DF.loc[DF['RIVER'] == river, ['STATION CODE', 'LOCATIONS']].sort_values(by=['STATION CODE'])
    stations.reset_index(inplace=True, drop=True)
    return jsonify({'stations': stations.to_dict('index')})
...
@app.route('/wqi-r/<station_code>', methods=['GET'])
def get_wqir(station_code):
    wqi = DF.loc[DF['STATION CODE'] == int(station_code), 'WQI'].values[0]
    return jsonify({"WQI": wqi})
